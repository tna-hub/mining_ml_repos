    ...
    def __init__(self, node=None, **kwargs):
        ...
        if self.cls == 'Name':
            ...
            if isinstance(node.op, ast.Add):
                constants = self.set_constants(node)
                self.value = self.set_binop_value(constants)

    def set_constants(self, node, constants=[]):
        lhs = node.left
        rhs = node.right
        if isinstance(node.op, ast.Add):
            ...
            if isinstance(lhs, ast.Constant):
                ...
                if constants is not None:
                    self.set_constants(node=lhs, constants=constants)
            else:
                constants = None
        else:
            constants = None
